...
def _input_fn(file_pattern, tf_transform_output, batch_size=_BATCH_SIZE):
    """Read gzip-compressed transformed TFRecords produced by TFX Transform."""
...
def _build_model(learning_rate=1e-3, hidden_units=64, dropout=0.2):
    inputs = {
        transformed_name(k): tf.keras.Input(shape=(1,), name=transformed_name(k))
...
def run_fn(fn_args: FnArgs):
    ...
    model = _build_model(
        ...
        int(hp.get("hidden_units", 64)),
        float(hp.get("dropout", 0.2)),
    )
    model.fit(
        train_ds,
